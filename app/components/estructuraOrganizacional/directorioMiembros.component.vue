<template>
  <div class="font-sans text-center px-4 py-8 md:py-12">
    <div class="max-w-6xl mx-auto">
      ...
      <div class="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 max-w-4xl mx-auto mb-4">
        <div class="relative bg-white rounded-xl shadow-[0_4px_12px_rgba(0,0,0,0.03)] border border-gray-200/70 px-4 py-2 flex items-center">
          <select v-model="filtroNombre" class="w-full text-[10px] md:text-xs font-semibold text-gray-500 bg-transparent focus:outline-none appearance-none pr-6 cursor-pointer">
        ...
        <div class="relative bg-white rounded-xl shadow-[0_4px_12px_rgba(0,0,0,0.03)] border border-gray-200/70 px-4 py-2 flex items-center">
          <select class="w-full text-[10px] md:text-xs font-semibold text-gray-400 bg-transparent focus:outline-none appearance-none pr-6 cursor-pointer">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 max-w-4xl mx-auto items-stretch mt-8 md:mt-10">
        <div 
          v-for="(uni, index) in universidadesFiltradas" 
          :key="index"
          class="bg-white rounded-2xl shadow-[0_10px_25px_rgba(0,0,0,0.03)] border border-gray-100 p-4 md:p-5 flex items-center text-left w-full min-h-[100px] md:h-32"
        >
          <div class="flex-shrink-0 w-24 md:w-32 h-full flex items-center justify-center p-2">
            <div v-if="uni.esImagenSolida" class="w-full h-16 md:h-full rounded-xl bg-[#d91d36] flex items-center justify-center text-white font-bold text-xs md:text-sm tracking-widest shadow-sm">
              {{ uni.siglas }}
            </div>
            <img v-else :src="uni.logo" :alt="uni.nombre" class="w-full h-full object-contain max-h-20 md:max-h-24" />
          </div>

          <div :class="[index % 2 === 0 ? 'bg-[#2b4c99]' : 'bg-[#d91d36]', 'w-[1.5px] h-16 md:h-20 flex-shrink-0 ml-3 md:ml-4']"></div>

          <div class="ml-4 md:ml-5 flex-1 flex flex-col justify-center">
            <span class="text-[9px] md:text-[11px] text-gray-500 underline decoration-gray-300 font-semibold block mb-0.5">
              Universidad Nacional
            </span>
            <h3 class="text-xs md:text-sm font-bold text-gray-900 leading-tight">
              {{ uni.nombre }}
            </h3>
          </div>
        </div>
      ...
      <div v-if="limite < universidades.length" class="mt-12">
        <button 
          @click="verMas" 
          class="px-8 md:px-10 py-3 bg-gradient-to-r from-[#d91d36] to-[#2b4c99] text-white font-bold text-[10px] md:text-xs rounded-xl shadow-md hover:opacity-90 transition-opacity uppercase tracking-wider"
        >
          Ver Más
        </button>
      </div>
